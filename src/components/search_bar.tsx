...
export default function displaySearchBar(searchValue: string, changeSearchEvent: (e: React.ChangeEvent<HTMLInputElement>) => void, changeGenderEvent: (filter: string) => void, filterValue: string) {
    const buttons = [
    ...
    function buttonStyle(gender: string): string {
        const classText = "gender-button " + gender + " " + (filterValue === gender ? "underline" : "");
        return classText;
    };
    ...
    return (
        <div className="input-container">
            ...
            {buttons.map(({gender, altText, url}) => 
                <div className={buttonStyle(gender)} onClick={() => changeGenderEvent(gender)}><img src={url} alt={altText}/></div>
            )}
        </div>
    )
}